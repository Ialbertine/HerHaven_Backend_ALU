...
const appointmentController = {
  ...
  bookAppointment: async (req, res) => {
    try {
      ...
      // Populate counselor and user details for response
      await appointment.populate('counselor', 'username firstName lastName specialization');
      ...
      logger.info(`New appointment booked: User ${req.user.username} with Counselor ${counselor.username}`);
  ...
  confirmAppointment: async (req, res) => {
    try {
      ...
      logger.info(`Appointment confirmed: ${appointmentId} by counselor ${req.counselor.username}`);
  ...
  rejectAppointment: async (req, res) => {
    try {
      ...
      appointment.cancellationReason = reason || 'Appointment rejected by counselor';
      appointment.cancelledAt = new Date();
      await appointment.save();

      logger.info(`Appointment rejected: ${appointmentId} by counselor ${req.counselor.username}`);
  ...
  cancelAppointment: async (req, res) => {
    try {
      ...
      logger.info(`Appointment cancelled: ${appointmentId} by user ${req.user.username}`);
  ...
  getAvailableTimeSlots: async (req, res) => {
    try {
      ...
      for (let hour = startHour; hour < endHour; hour++) {
        ...
        const isBooked = bookedAppointments.some(apt => apt.appointmentTime === timeSlot);
  ...
  startSession: async (req, res) => {
    try {
      ...
      if (minutesUntilStart > 15) {
        return res.status(400).json({
          success: false,
          message: 'Session can only be started 15 minutes before scheduled time'
        });
      }
      ...
      await notificationController.createNotification(
        ...
        'session_starting',
        'Session Starting',
        `Your session with ${req.counselor.firstName} ${req.counselor.lastName} is starting now.`,
        {
          appointment: appointment._id,
          counselor: req.counselor._id,
          channels: ['in-app'],
          data: {
      ...
      logger.info(`Session started: ${appointmentId} by counselor ${req.counselor.username}`);
  ...
  endSession: async (req, res) => {
    try {
      ...
      await notificationController.createNotification(
        ...
        'session_completed',
        'Session Completed',
        `Your session with ${req.counselor.firstName} ${req.counselor.lastName} has been completed.`,
        {
          appointment: appointment._id,
          counselor: req.counselor._id,
          channels: ['in-app']
      ...
      logger.info(`Session ended: ${appointmentId} by counselor ${req.counselor.username}`);
